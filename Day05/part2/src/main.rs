...
fn main() -> std::io::Result<()> {
    ...
    for line in file_lines {
        ...
        for j in 0..instr[0] {
            ...
            temp_deq.push_front(a);
        }

        for j in 0..instr[0] {
            let b = temp_deq.pop_front().unwrap();
            
            stacks[instr[2] - 1].push_front(b);
        }
...
I was lazy and just added a temporary Deque to place items on while rearranging
the items. This allows the items to stay in the same order when moving as a group.
*/
